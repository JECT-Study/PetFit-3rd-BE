...
public class TodayRoutineFacade {
     ...
     * 3. 오늘의 루틴 메모 여부 저장
     * 4. 오늘의 루틴 완료 여부 저장
     * 5. UNCHECKED 상태의 루틴들을 DB에 저장
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public boolean todayRoutineSave(Entry entry){
        /** 2. CHECKED와 MEMO한 루틴이 없다면 루틴 미완료로 저장 및 종료 */
        ...
        /**
         * 4. 오늘의 루틴 완료 여부 저장
         * DB에 저장된 오늘의 루틴 개수와 활성화된 옵션 개수가 같다면 루틴 완료로 업데이트하고 종료
        ...
        for (String category : activatedSlotOptions) {
            Integer targetAmount = slotQueryService.getTargetAmountOrNull(slot, category);
